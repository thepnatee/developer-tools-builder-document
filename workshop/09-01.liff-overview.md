# Workshop: LINE Front-end Framework (LIFF)
...
1. ผู้ใช้เปิด LIFF URL จากห้องแชท
2. LIFF App เรียก `liff.init({ liffId })`
3. LIFF SDK ขอ access token + id token จาก LINE Platform
4. LINE Platform ส่ง credentials กลับมาใน primary redirect URL
5. LIFF SDK resolve Promise (init สำเร็จ)
6. LIFF App เช็ค `liff.isLoggedIn()`:
   - **ยังไม่ login** → LIFF SDK redirect ไปหน้า LINE Login
   - **login แล้ว** → LIFF App เรียก `liff.getProfile()` และได้ `{ userId, displayName, pictureUrl, ... }` กลับมา
...
> **อัปเดต (พ.ค. 2026):** ตั้งแต่ LINE v26.7.0 ขึ้นไป header ของ LIFF browser และ LINE MINI App ถูกปรับ spec ใหม่ รวมถึง Action Button behavior — หากแอปของคุณพึ่งพา Action Button UI ควรทดสอบบน LINE เวอร์ชันล่าสุดก่อน deploy
...
| เวอร์ชัน | Method | รายละเอียด |
|---------|--------|------------|
| LIFF v2.27.0 (มิ.ย. 2025) | `liff.permission.getGrantedAll()` | ดู permission scope ทั้งหมดที่ผู้ใช้ grant ไว้แล้ว |
| LIFF v2.28.0 (มี.ค. 2026) | `liff.requestFriendship()` | ให้ผู้ใช้ add LINE OA เป็นเพื่อนจากภายใน LIFF ได้ทันที |
...
- **พลาด:** เรียก `liff.openWindow()` แล้วพฤติกรรมเปลี่ยนไปหลัง LINE v15.20.0+ (ธ.ค. 2025)
  **ถูก:** ตั้งแต่ LINE v15.20.0+ `liff.openWindow()` จะเปิด Universal Link / App Link แตกต่างออกไป — ถ้าแอปคุณเรียกฟังก์ชันนี้เพื่อเปิด URL ภายนอก ให้ทดสอบกับ LINE เวอร์ชันล่าสุดและอัปเดต behavior ตามความเหมาะสม

- **พลาด:** ปุ่มหรือ content ที่วางชิดล่างสุดของหน้าจอถูก nav bar ทับ (Android LINE v26+, มี.ค. 2026)
  **ถูก:** Android เปิด edge-to-edge display ทำให้ system navigation bar ทับส่วนล่างของ LIFF — เพิ่ม padding สำหรับ system insets เช่น `padding-bottom: env(safe-area-inset-bottom)` ใน CSS เพื่อป้องกัน UI โดนบัง

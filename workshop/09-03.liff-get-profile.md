...
1. LIFF App เรียก `liff.init({ liffId })` → SDK resolve init
2. LIFF App เช็ค `liff.isLoggedIn()`:
   - **ยังไม่ login (false)** → เรียก `liff.login({ redirectUri })` → SDK ขอ consent (scope: profile, openid, email) จาก LINE Platform → LINE แสดงหน้า consent → ผู้ใช้อนุญาต → LINE redirect กลับพร้อม access_token
3. LIFF App เรียก `liff.getProfile()`
4. SDK ดึงโปรไฟล์จาก LINE Platform (Bearer token)
5. LINE ส่ง `{ userId, displayName, pictureUrl, statusMessage }` กลับมา
6. LIFF App เรียก `liff.getDecodedIDToken()` → ได้ `{ email, sub, name, ... }`

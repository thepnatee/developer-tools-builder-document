...
การเลือก Domain และ Endpoint ของ Messaging API:

- **ส่งข้อความ / สติ๊กเกอร์ / รูป** → ใช้ `api.line.me`
  - `/v2/bot/message/reply` — ตอบกลับด้วย replyToken
  - `/v2/bot/message/push` — ส่งหาผู้ใช้คนเดียว
  - `/v2/bot/message/multicast` — ส่งหาหลายคนพร้อมกัน
  - `/v2/bot/message/broadcast` — ส่งหาทุกคน
  - `/v2/bot/message/narrowcast` — ส่งแบบกำหนดกลุ่มเป้าหมาย

- **อัปโหลด/ดาวน์โหลดไฟล์ใหญ่** (รูป, วิดีโอ, ริชเมนู) → ใช้ `api-data.line.me`

ประเภท message ที่รองรับ: `text` / `sticker` / `image` / `video` / `audio` / `location` / `imagemap` / `template` / `flex` / `textV2` / `coupon`
...
[Generate Image](https://devtoolsbuilder.cloudea.tech/)
```json
...
- [Sticker List](https://developers.line.biz/en/docs/messaging-api/sticker-list/)
- [Flex Simulator](https://developers.line.biz/flex-simulator/)
- [Imagemap Generator](https://devtoolsbuilder.cloudea.tech/)

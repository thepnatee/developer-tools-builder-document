...
ขั้นตอนการใช้งาน LIFF CLI:

1. `npm install -g @line/liff-cli` — ติดตั้ง CLI
2. `liff-cli channel add` — ผูก LINE Login Channel
3. `liff-cli channel use` (optional) — ตั้ง default channel
4. เลือก Operation:
   - **Create** → `liff-cli app create --name --endpoint-url --view-type`
   - **Update** → `liff-cli app update --liff-id ...`
   - **List** → `liff-cli app list`
   - **Delete** → `liff-cli app delete --liff-id`

ทุก operation สำเร็จจะแสดง "Successfully created/updated/deleted LIFF app: liff-id"
...
- **พลาด:** Endpoint URL เป็น `http://` (ไม่ใช่ HTTPS) → CLI reject
  **ถูก:** ใช้ `https://` เท่านั้น — ถ้า dev local ใช้ ngrok สร้าง HTTPS tunnel (LIFF CLI v0.4+ รองรับ ngrok อย่างเป็นทางการ ดูด้านล่าง)
...
- **พลาด:** ใช้ Node.js v18 แล้ว CLI install ไม่สำเร็จ
  **ถูก:** อัพเป็น v20 ขึ้นไป — `nvm install 20 && nvm use 20`

## ngrok Integration (LIFF CLI v0.4+)

> **อัปเดต (มิ.ย. 2025):** LIFF CLI v0.4.0 เพิ่ม **official ngrok support** — สามารถรัน LIFF dev server พร้อม HTTPS tunnel ได้ในคำสั่งเดียว โดยไม่ต้องเปิด ngrok แยก

```shell
# ติดตั้ง LIFF CLI เวอร์ชันล่าสุด
npm install -g @line/liff-cli

# รัน dev server + ngrok tunnel พร้อมกัน
liff-cli serve --liff-id 1234567890-AbcdEfgh --port 3000
# LIFF CLI จะสร้าง ngrok URL อัตโนมัติและ update endpoint ใน Console
```

ก่อนหน้านี้ต้องรัน ngrok แยก แล้วอัปเดต endpoint URL ด้วยตัวเอง — v0.4+ รวมขั้นตอนนี้ไว้ในคำสั่งเดียว ทำให้ dev loop เร็วขึ้น

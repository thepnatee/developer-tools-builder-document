...
ลำดับการทำงานของ Messaging API:

1. **ผู้ใช้ LINE** ส่งข้อความถึง LINE OA
2. **LINE Platform** ส่ง Webhook Event + replyToken ไปยัง Bot Server
3. **Bot Server** ประมวลผลข้อความ จากนั้นเลือกวิธีตอบกลับ:
   - **2-way (Reply)** — POST /v2/bot/message/reply ด้วย replyToken (ฟรี 1 message สูงสุด 5 bubbles)
   - **1-way (Push/Broadcast)** — POST /v2/bot/message/push หรือ broadcast (หักโควต้าตามจำนวนผู้รับ)
4. **LINE Platform** ส่งข้อความกลับไปยังผู้ใช้

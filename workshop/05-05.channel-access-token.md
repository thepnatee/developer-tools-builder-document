...
**Channel Access Token** มี 4 ประเภท:

- **Long-Lived** — ไม่หมดอายุ → ง่ายสุด แต่ปลอดภัยน้อยสุด
- **Short-Lived** — 30 วัน → ปลอดภัยปานกลาง ต้องจัดการเอง
- **v2.1** — ยาวสุด 30 วัน + JWT → ปลอดภัยสูงสุด แนะนำสำหรับ Production
- **Stateless** — 15 นาที ไม่จำกัดจำนวน → per-request ไม่ต้องเก็บ Token
...
1. นักพัฒนา Issue Token ด้วย Channel ID + Secret (หรือ JWT) ไปยัง LINE Platform
2. LINE Platform ส่ง `access_token` + `expires_in` กลับมา
3. นักพัฒนาเก็บ Token ใน env variable / Secret Manager ไปยัง Bot Server
4. Bot Server ใช้ Token ยิง API ซ้ำได้หลายครั้ง
5. ก่อนหมดอายุ — นักพัฒนา Issue Token ใหม่ (rotate)
6. นักพัฒนา Revoke Token เก่า (ถ้าต้องการเปลี่ยนทันที)
7. LINE Platform ตอบ 200 OK

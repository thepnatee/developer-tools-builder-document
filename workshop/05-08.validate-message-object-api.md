# Workshop: Validate Message Object API
...
ขั้นตอนการใช้ Validate API ก่อนส่งจริง:

1. เตรียม Message JSON
2. ส่งไปยัง Validate API:
   - **200 OK** → JSON ถูกต้อง → เข้า Queue / Job Scheduler → ส่งจริงด้วย Reply / Push / Multicast / Narrowcast / Broadcast → ผู้ใช้ LINE รับข้อความ
   - **400 Bad Request** → JSON ผิด → กลับไปแก้ไข JSON แล้วส่ง Validate ใหม่

...
วงจรชีวิตของ Webhook — ลำดับเหตุการณ์:

**กรณีปกติ (ตอบภายใน timeout):**
1. ผู้ใช้ พิมพ์ / กดปุ่ม / เพิ่มเพื่อน
2. LINE Platform ส่ง POST Webhook Event (JSON + replyToken) ไปยัง Bot Server
3. Bot Server บันทึก event ลงฐานข้อมูล
4. Bot Server ตอบกลับ 200 OK
5. Bot Server ส่ง POST /message/reply (ใช้ replyToken)
6. LINE Platform ส่งข้อความตอบกลับไปยังผู้ใช้

**กรณีตอบพลาด / timeout (ไม่ใช่ 2xx):**
1. Bot Server ตอบไม่ทัน หรือ ส่ง 5xx
2. LINE Platform ส่ง Webhook Event ซ้ำ (isRedelivery: true, webhookEventId เดิม) — เมื่อเปิด Webhook Redelivery
3. Bot Server ตรวจ webhookEventId ซ้ำ (idempotency) แล้วตอบ 200 OK

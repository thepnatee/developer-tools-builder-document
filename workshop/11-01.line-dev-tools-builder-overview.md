...
**ผู้ใช้งาน** (Developer / Marketer)
→ Login ผ่าน LINE LIFF + Firebase Auth
→ เข้าสู่ **LINE Developer Tools Builder**

แพลตฟอร์มแบ่งออกเป็น 3 กลุ่มเครื่องมือ ที่ทั้งหมดเชื่อมต่อกับ LINE Messaging API:

- **Core Tools** — Rich Menu Builder, Flex Message, Imagemap, Send Message, Member Management, Chat Mode, Webhook Proxy, Cronjob, Bot Templates, Live Polling, Quiz Game
- **Marketplace Apps** — Shop/POS, Loyalty Program, Booking
- **Utility Tools** — Image Upload/Resize, QR/PromptPay/URL Scheme, Short Links

LINE Messaging API → LINE OA (ส่งข้อความไปหาผู้ใช้)
LINE Webhook → Webhook Proxy → Developer Backend (optional, สำหรับ forward events)

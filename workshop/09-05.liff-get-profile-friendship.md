...
1. LIFF page โหลด → เรียก `liff.init`
2. ตรวจสอบ `logged in?`:
   - **No** → เรียก `liff.login`
   - **Yes** → เรียก `liff.getFriendship`
3. ตรวจสอบ `friendFlag`:
   - **true** → แสดง UI หลักเลย
   - **false** → แสดงปุ่มเพิ่มเพื่อน LINE OA → เมื่อผู้ใช้กดเพิ่มเพื่อนแล้ว → แสดง UI หลัก

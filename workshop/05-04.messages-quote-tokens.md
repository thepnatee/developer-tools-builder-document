...
ขั้นตอนการใช้งาน Quote Token:

1. ผู้ใช้ส่งข้อความ "ราคาเท่าไหร่?"
2. LINE Platform ส่ง Webhook event (มี `quoteToken` แนบมา) ไปยัง LINE Bot
3. Bot เก็บ `quoteToken` ไว้ใช้อ้างอิง
4. Bot ส่ง Reply/Push message พร้อม `quoteToken` ของข้อความเดิม
5. LINE Platform แสดงข้อความตอบกลับ พร้อมอ้างอิงข้อความ "ราคาเท่าไหร่?"
...
เทียบง่าย ๆ: ถ้า replyToken คือ "ตั๋วหนังรอบเดียวใช้แล้วทิ้งภายใน 20 นาที" — quoteToken ก็คือ "ตั๋วถาวร" ที่จะเก็บไว้ใช้เมื่อไหร่ก็ได้ และใช้ซ้ำได้
...
- **พลาด:** สับสนระหว่าง `replyToken` (ใน reply API) กับ `quoteToken`
  **ถูก:** สองตัวต่างกัน: `replyToken` ใช้ **ครั้งเดียว ภายใน 20 นาที** สำหรับเรียก `/v2/bot/message/reply` ส่วน `quoteToken` ใช้เพื่อ **อ้างอิง** ข้อความในเนื้อหา message

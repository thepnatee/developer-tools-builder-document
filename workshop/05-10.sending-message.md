...
วิธีเลือกรูปแบบการส่งข้อความ:

1. ผู้ใช้เพิ่งทัก / มี replyToken → ใช้ **Reply** (ฟรี ไม่หักโควตา)
2. ต้องส่งก่อน / ไม่มี replyToken → เลือกตามกลุ่มผู้รับ:
   - **ส่งหา 1 คน** → ใช้ **Push** (2,000 req/sec)
   - **ส่งหาหลายคน แบบกำหนด ID** → ใช้ **Multicast** (200 req/sec, สูงสุด 500 IDs ต่อครั้ง)
   - **ส่งหาทุก friend ของ OA** → ใช้ **Broadcast** (60 req/hour)
   - **ส่งหากลุ่มเจาะจง (demographic/audience)** → ใช้ **Narrowcast** (ดู 05-11)
...
## notificationDisabled — ปิด push notification บนมือถือ

`notificationDisabled` เป็น optional field ใน request body ของทุก send endpoint (push, multicast, broadcast, narrowcast) ที่บอกว่า **จะให้ LINE ส่ง push notification ไปที่มือถือผู้ใช้ด้วยไหม**

| ค่า | ผลลัพธ์ |
|-----|---------|
| `false` (default) | ผู้ใช้ได้รับ push notification ตามปกติ (เสียงดัง / badge เพิ่ม) |
| `true` | ข้อความถูกส่งและแสดงในแชท **แต่ไม่มี notification** — มือถือเงียบ ไม่ดัง ไม่มี badge |

**ใช้ `true` เมื่อ:**
- ส่งข้อความเบื้องหลังที่ผู้ใช้ไม่ต้องรับรู้ทันที เช่น silent update, log
- ส่งข้อความเยอะในช่วงเวลาสั้น และกลัวว่าผู้ใช้จะถูก spam notification จน unfollow
- ส่งสรุปรายวันในเวลาดึกที่ไม่ต้องการปลุกผู้ใช้
...
-d '{
    ...
    "messages":[
        {
            "type":"text",
            "text":"อัปเดตข้อมูลเงียบๆ นะครับ"
        }
    ],
...
> **หมายเหตุ:** field นี้ควบคุมแค่ว่า LINE จะส่ง notification ออกไปไหม — ถ้าผู้ใช้ปิด notification ใน LINE app หรือใน device settings อยู่แล้ว ก็ไม่มี notification อยู่แล้วโดยไม่ต้องตั้ง field นี้
...
- **พลาด:** ใช้ Reply แบบผ่านไปแล้ว 5 นาทีค่อยตอบ
  **ถูก:** `replyToken` มีอายุ 20 นาที และใช้ได้ครั้งเดียว — ถ้าเกินต้องใช้ Push
...
- **พลาด:** ส่ง broadcast แคมเปญโปรโมชัน 50 ข้อความในครั้งเดียวโดยไม่ตั้ง `notificationDisabled: true` แล้วผู้ใช้รำคาญ notification ท่วมมือถือจน unfollow
  **ถูก:** ถ้าต้องส่งหลายข้อความติดกัน ตั้ง `notificationDisabled: true` ในข้อความกลางๆ และเปิด notification เฉพาะข้อความสุดท้ายเท่านั้น
...
- [ ] พิจารณาตั้ง `notificationDisabled: true` เมื่อส่งข้อความหลายอันต่อกัน หรือส่งในเวลาที่ไม่ต้องการปลุกผู้ใช้

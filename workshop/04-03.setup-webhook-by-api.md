...
การจัดการ Webhook ผ่าน API โดยใช้ Channel Access Token:

- **ตั้ง/อัปเดต URL** → `PUT /v2/bot/channel/webhook/endpoint`
  - LINE บันทึก URL ใหม่

- **ดู URL ปัจจุบัน** → `GET /v2/bot/channel/webhook/endpoint`
  - ได้ endpoint + active status กลับมา

- **ทดสอบว่าใช้งานได้** → `POST /v2/bot/channel/webhook/test`
  - LINE ยิง event ตัวอย่างไปหา URL
  - URL ตอบ 200 → `success: true`
  - URL ไม่ตอบ 200 → `success: false` + reason

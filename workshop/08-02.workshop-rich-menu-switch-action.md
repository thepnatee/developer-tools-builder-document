...
1. ผู้ใช้กดปุ่มใน Rich Menu (message/postback) ส่งคำขอไปยัง LINE Server
2. LINE Server ส่ง Webhook Event ไปยัง Bot Application
3. Bot Application ส่งคำขอ `POST /v2/bot/user/{userId}/richmenu/{richMenuId}` กลับไปยัง LINE Server
4. LINE Server สลับ Rich Menu ให้ผู้ใช้
...
1. ผู้ใช้กดปุ่มที่มี richmenuswitch action (ระบุ alias) ส่งคำขอไปยัง LINE Server
2. LINE Server สลับ Rich Menu ตาม alias ให้ผู้ใช้ทันที
...
การสลับ Rich Menu A กับ B ด้วย Alias ทำงานแบบนี้:

- **Rich Menu A** (ค่าเริ่มต้น)
  - alias: `richmenu-alias-a`
  - มีปุ่ม [ไปแท็บ B] — action: richmenuswitch, richMenuAliasId: `richmenu-alias-b`
  - ผู้ใช้กดปุ่มนี้ → สลับไป **Rich Menu B**

- **Rich Menu B**
  - alias: `richmenu-alias-b`
  - มีปุ่ม [ไปแท็บ A] — action: richmenuswitch, richMenuAliasId: `richmenu-alias-a`
  - ผู้ใช้กดปุ่มนี้ → สลับกลับไป **Rich Menu A**
...
ลำดับขั้นตอนการสร้าง Rich Menu แบบสลับแท็บ:

1. เตรียมภาพ Rich Menu A และ B
2. สร้าง Rich Menu A (มี action: richmenuswitch → alias-b)
3. อัปโหลดภาพสำหรับ Rich Menu A
4. สร้าง Rich Menu B (มี action: richmenuswitch → alias-a)
5. อัปโหลดภาพสำหรับ Rich Menu B
6. ตั้งค่า Rich Menu A เป็น Default
7. สร้าง Alias ของ Rich Menu A (`richmenu-alias-a`)
8. สร้าง Alias ของ Rich Menu B (`richmenu-alias-b`)

→ พร้อมใช้งาน: ผู้ใช้สลับแท็บได้ทันที

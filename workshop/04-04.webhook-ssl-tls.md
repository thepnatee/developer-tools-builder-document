...
กระบวนการตรวจสอบ SSL/TLS เมื่อ LINE ยิง webhook:

1. LINE ยิง webhook ไปที่ URL ของคุณ → เริ่ม TLS handshake
2. ตรวจสอบเวอร์ชัน TLS:
   - **TLS 1.1 หรือต่ำกว่า** → ปฏิเสธทันที
   - **TLS 1.2 / 1.3** → ตรวจสอบ Cipher suite ต่อ
3. ตรวจสอบ Cipher suite:
   - **ไม่รองรับ** → ปฏิเสธ
   - **รองรับ** → ตรวจสอบใบรับรอง
4. ตรวจสอบใบรับรอง:
   - **Self-signed** → ปฏิเสธ
   - **จาก Public CA** → ตรวจสอบ Certificate chain
5. ตรวจสอบ Certificate chain:
   - **ขาด intermediate** → ปฏิเสธ
   - **ครบ** → ส่ง request ต่อไปยัง webhook → Webhook ตอบ 200 OK

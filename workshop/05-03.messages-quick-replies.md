...
การทำงานของ Quick Reply:

1. Bot ส่งข้อความพร้อม `quickReply`
2. แสดงปุ่มแถวล่างของแชท
3. ผู้ใช้แตะปุ่ม → ผลลัพธ์ตามประเภท action:
   - **Message action** → ส่ง text ให้บอท → ปุ่มหายไป
   - **Postback action** → ส่ง data ให้บอท → ปุ่มหายไป
   - **Camera / Gallery** → เปิดกล้อง/คลังภาพ → ปุ่มยังอยู่จนส่ง data จริง
   - **Location** → ส่งพิกัด → ปุ่มยังอยู่จนส่ง data จริง
   - **Datetime picker** → เลือกวัน/เวลา → ปุ่มยังอยู่จนส่ง data จริง
   - **URI** → เปิดลิงก์ภายนอก → ปุ่มหายไป
   - **Clipboard** → คัดลอกข้อความ → ปุ่มหายไป

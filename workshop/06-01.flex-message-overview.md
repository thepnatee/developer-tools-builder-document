# Flex Message — ออกแบบการ์ดสวย ๆ ใน LINE ด้วย JSON

> ถ้าเคยเห็นบอท LINE ส่ง "บัตรสมาชิก", "ใบเสร็จร้าน", "เมนูอาหาร" หรือ "สลิปโอนเงิน" ที่มีปุ่มกดได้ — ทั้งหมดนั้นคือ **Flex Message** ทั้งหมด มันคือของเล่นที่สนุกที่สุดของ LINE Messaging API เพราะเราออกแบบเองได้เหมือน HTML/CSS แต่ใช้แค่ JSON
...
บอท LINE ที่ส่งแค่ข้อความตัวอักษรธรรมดา "ดูน่าเบื่อ" มาก — ผู้ใช้อยากเห็น UI ที่สวยงาม มีปุ่มกดได้ และแยกข้อมูลเป็นชิ้น ๆ Flex Message คือคำตอบของปัญหานี้

**ประโยชน์จริง:**
- สร้างการ์ดสินค้า/บริการในรูปแบบ carousel ให้ผู้ใช้เลื่อนเลือก
- ออกใบเสร็จ/สลิป/บัตรสมาชิกให้ดูเป็นทางการ
- ใส่ปุ่มหลายปุ่มในข้อความเดียว ประหยัด quota
- จัดสีและ layout ตามแบรนด์ได้อย่างอิสระ

## ภาพรวมโครงสร้าง Flex Message

**Flex Message** (type: flex) ประกอบด้วย:
- **altText** — ข้อความแสดงใน push notification
- **contents** — เนื้อหาหลักของ Flex Message
  - **Container** — กล่องหลัก มี 2 แบบ:
    - **bubble** (สูงสุด 30KB) — กล่องเดียว
    - **carousel** (สูงสุด 50KB, ไม่เกิน 12 bubbles) — หลายกล่องเลื่อนได้
  - **Blocks** ภายใน bubble:
    - header
    - hero
    - body
    - footer
    - styles
  - **Components** ภายใน body (และ block อื่น ๆ):
    - box
    - text
    - image
    - video
...
> **อ่านง่าย ๆ:** Flex Message = ซองจดหมาย → Container = กล่องข้างใน → Block = ชั้นของกล่อง (หัว/ตัว/ท้าย) → Component = ของชิ้นเล็กที่วางในแต่ละชั้น
...
- **พลาด:** ส่ง Flex Message ไม่มี `altText` ทำให้ push notification ไม่มีข้อความแสดง
  **ถูก:** ใส่ `altText` ที่บอกเนื้อหาคร่าว ๆ ไว้เสมอ (เช่น "ใบเสร็จการสั่งซื้อ")

- **พลาด:** ใช้รูปจากเว็บที่ไม่รองรับ HTTPS หรือ hotlink แล้วรูปไม่ขึ้น
  **ถูก:** อัปโหลดรูปไป Firebase Storage หรือ imgbb ก่อน แล้วใช้ URL HTTPS เสมอ

- **พลาด:** สร้าง carousel ที่มี bubble เกิน 12 หรือขนาด JSON เกิน 50KB
  **ถูก:** carousel จำกัดไว้ที่ 12 bubbles และขนาดรวมไม่เกิน 50KB ถ้าเกินต้องแบ่งเป็นหลายข้อความ

- **พลาด:** ใช้ property ใหม่ (เช่น `maxWidth`, `lineSpacing`) แล้วผู้ใช้ LINE เวอร์ชันเก่าเห็น layout พัง
  **ถูก:** เช็คเวอร์ชันขั้นต่ำในตาราง Operating Environment ก่อนใช้ property ใหม่

- **พลาด:** ใส่ข้อความยาวใน text component โดยไม่ใส่ `wrap: true` ทำให้โดนตัดด้วย …
  **ถูก:** text ที่ยาวเกินบรรทัดเดียวต้องใส่ `"wrap": true`

## Checklist ก่อนไปต่อ

- [ ] เข้าใจความต่างระหว่าง bubble กับ carousel (ขนาดและจำนวน bubble)
- [ ] รู้จัก block ทั้ง 5: header, hero, body, footer, styles
- [ ] รู้ว่า baseline box ใส่อะไรได้/ไม่ได้ (icon ได้, box/button/image ไม่ได้)
- [ ] ทดลองสร้าง bubble แรกใน [Flex Simulator](https://developers.line.biz/flex-simulator/)
- [ ] ลองปรับ `size`, `margin`, `spacing` ดูผลต่างระหว่างค่า xs-xxl
...
- [CSS Flexible Box (CSS Flexbox)](https://www.w3.org/TR/css-flexbox-1/)

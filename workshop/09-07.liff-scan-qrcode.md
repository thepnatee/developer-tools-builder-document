...
1. ผู้ใช้กดปุ่ม "สแกน" ใน LIFF Page
2. LIFF Page เช็ค `isInClient` + LIFF size = full
3. LIFF Page เรียก `liff.scanCodeV2()` เพื่อเปิดกล้อง
4. กล้องมือถือเปิดขึ้น พร้อมกรอบจ่อ QR
5. ผู้ใช้จ่อกล้องที่ QR
6. กล้องส่ง result `{value: "PRODUCT-123"}` กลับไปยัง LIFF Page
7. LIFF Page ส่งคำขอ `GET /products/PRODUCT-123` ไปยัง Backend
8. Backend ส่ง `{name, price, stock}` กลับมา
9. LIFF Page แสดงข้อมูลสินค้าให้ผู้ใช้

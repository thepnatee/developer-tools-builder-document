...
**Rich Menu (1 ชิ้น)** ประกอบด้วย:

- **Rich Menu Image** — ไฟล์ JPEG/PNG ขนาด 2500x1686 px ที่ใช้เป็นพื้นหลัง
- **Tappable Areas** — พื้นที่กดได้สูงสุด 20 จุด โดยแต่ละจุดใช้พิกัดบนภาพเดียวกัน เช่น:
  - Area 1: x:0, y:0, w:833, h:843 → action: URI
  - Area 2: x:833, y:0, w:834, h:843 → action: message
  - Area 3: x:1667, y:0, w:833, h:843 → action: postback
  - Area 4: x:0, y:843, w:1250, h:843 → action: richmenuswitch
  - Area 5: x:1250, y:843, w:1250, h:843 → action: datetimepicker
- **Chat Bar Text** — แถบเปิด/ปิดเมนูที่ด้านล่างของหน้าจอ
...
วิธีเลือกเครื่องมือสร้าง Rich Menu:

- **ต้องการดูสถิติคลิก/แสดงผล** → ใช้ **LINE OA Manager** (GUI, ตั้งเวลาได้, ดูสถิติได้)
- **ไม่จำเป็นต้องดูสถิติ** → ตรวจสอบต่อ:
  - **ต้องใช้ Postback, Datetime หรือ Switch Action** → ใช้ **Messaging API** (ปรับแต่งได้เต็มที่)
  - **ไม่ต้องใช้** → ใช้ **LINE OA Manager** ก็เพียงพอ

> หมายเหตุ: Rich Menu ที่สร้างด้วยเครื่องมือใดจะถูกล็อคกับเครื่องมือนั้น — ไม่สามารถสลับไปแก้ด้วยเครื่องมืออื่นได้
...
2. **ใช้ Endpoint สร้าง Rich Menu**: เรียกใช้ Endpoint [Create rich menu](https://developers.line.biz/en/reference/messaging-api/#create-rich-menu) เพื่อสร้าง Rich Menu ขึ้นมา
    ...
    ##### ข้อจำกัด
    - Rate limit 100 requests per hour (Create rich menu)
    - **`GET /richmenu/list` — rate limit เปลี่ยนแล้ว (พ.ค. 2026):** ตรวจสอบ rate limit ปัจจุบันที่ [Messaging API reference](https://developers.line.biz/en/reference/messaging-api/#get-rich-menu-list) ก่อนเรียก endpoint นี้ในลูป

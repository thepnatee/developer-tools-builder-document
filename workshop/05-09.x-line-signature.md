...
ขั้นตอน HMAC Signature Verification — ทั้งสองฝั่งถือ Channel Secret เดียวกัน:

**ฝั่ง LINE Platform (ผู้ส่ง):**
1. ผู้ใช้ส่งข้อความหา OA
2. LINE สร้าง webhook body (JSON)
3. คำนวณ HMAC-SHA256(body, channelSecret)
4. เข้ารหัส Base64 ได้ signature
5. ส่ง `POST /webhook` พร้อม Header `x-line-signature` และ Body `{events:[...]}`

**ฝั่ง Bot Server (ผู้รับ):**
6. อ่าน raw body (ห้าม parse ก่อน!)
7. คำนวณ HMAC-SHA256(body, channelSecret)
8. เข้ารหัส Base64 ได้ expectedSignature
9. เปรียบเทียบ:
   - **signature == expectedSignature** → ยืนยันว่ามาจาก LINE จริง → ประมวลผล webhook event → ตอบ 200 OK
   - **signature != expectedSignature** → อาจโดนปลอม/ดัดแปลง → ตอบ 401 Unauthorized หรือทิ้ง request
